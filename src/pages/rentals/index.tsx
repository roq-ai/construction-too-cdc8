  ...
  const columns: ColumnType[] = [
    {
      ...
      accessorKey: 'rental_start_date',
      cell: ({ row: { original: record } }: any) =>
        record?.rental_start_date ? format(parseISO(record?.rental_start_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'rental_end_date',
      header: 'Rental End Date',
      accessorKey: 'rental_end_date',
      cell: ({ row: { original: record } }: any) =>
        record?.rental_end_date ? format(parseISO(record?.rental_end_date as unknown as string), 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/customers/view/${record.customer?.id}`}>
              {record.customer?.rental_start_date}
            </Link>
          ),
        }
      : null,
